...
#define WP_LOGE(...)                             \
    do                                           \
    {                                            \
        fprintf(stderr, "[WorkerPool] ERROR: "); \
...
static void *worker_thread_func(void *arg)
{
     ...
     * On fresh boot all N worker threads start simultaneously, causing
     * heavy SQLite contention during Laravel's bootstrap phase and
     * transient memory spikes.  Stagger by 2 seconds per worker_id so
     * each thread's `require autoload.php` and `kernel->bootstrap()`
     * hit the filesystem / SQLite sequentially, reducing contention and
     * peak memory by ~30%. */
    if (worker_id > 0)
    {
        unsigned int stagger_us = (unsigned int)worker_id * 2000000u; /* 2s per slot */
        WP_LOGI("Worker %d: staggering start by %u ms", worker_id, stagger_us / 1000);

        /* Use condvar wait instead of usleep so we can be woken for shutdown */
        ...
        {
            WP_LOGI("Worker %d: pool stopped during stagger, exiting", worker_id);
            return NULL;
        }
    }
        ...
        if (job_status == JOB_STATUS_FAILED || exec_result != 0)
        {
            ...
            if (consecutive_crashes >= cb_max)
            {
                ...
                while (atomic_load(&pool->running) && pool->queue_head == NULL)
                {
                    ...
                    if (rc != 0)
                        break; /* Timeout expired or error */
                }
                pthread_mutex_unlock(&pool->queue_mutex);
...
int worker_pool_submit(worker_pool_t *pool, php_request_context_t *ctx)
{
    ...
    int priority = php_request_get_priority(ctx);
    ...
    /*
     * Priority insertion: higher priority jobs are placed before lower ones.
     * Jobs with equal priority maintain FIFO order (stable insertion).
     * Priority 0 (default) uses the fast path (append to tail).
     */
    if (priority <= 0 || pool->queue_head == NULL)
    {
        /* Normal priority or empty queue: append to tail (FIFO) */
        if (pool->queue_tail)
        {
    ...
    {
        /* Elevated priority: insert before the first node with lower priority.
         * Walk the queue to find the insertion point. */
        job_node_t *prev = NULL;
        job_node_t *cur = pool->queue_head;

        while (cur && php_request_get_priority(cur->ctx) >= priority)
        {
            prev = cur;
            cur = cur->next;
        }
...
int worker_pool_worker_count(const worker_pool_t *pool)
{
