...
#include <stdint.h>
    ...
    /**
     * Set the database path for native queue peek + connection pool.
     * Must be called before supervisor_start() for the sqlite pool
     * and native queue optimizations to take effect.
     *
     * When set, the supervisor will:
     *  1. Pre-open a pool of SQLite connections (WAL mode, busy_timeout)
     *  2. Use native_queue_peek() to skip expensive PHP bootstraps when
     *     the queue is empty (zero-PHP-overhead introspection)
     *
     * @param db_path  Absolute path to the SQLite database file
     * @param pool_size Number of pooled connections (0 = disable pooling)
     */
    void supervisor_set_db_path(const char *db_path, int pool_size);

    /**
     * Get native queue status as JSON (zero-PHP-overhead).
     * Uses the native queue module to query the jobs table directly.
     *
     * @return JSON string (caller must free), or NULL if db_path not set
     *         Format: {"total":N,"queues":{"default":N,"high":N,...}}
     */
